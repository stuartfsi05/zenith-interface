...
export class HeaderComponent {
  ...
  private bindEvents() {
    ...
    this.headerModelSelector.addEventListener('click', (e) => {
      e.stopPropagation();
      const isTransient = this.headerModelSelector!.querySelector('span')?.textContent === 'Zenith Engine (Transient)';
      if (isTransient) {
        Toast.show('Inicie um chat ou selecione um chat salvo para ver opções', 'info');
        return;
      }
    ...
    this.headerActionPin?.addEventListener('click', (e) => {
      ...
      const activeItem = document.querySelector('.history-item.active') as HTMLElement;
      if (activeItem) {
        const pinBtn = activeItem.querySelector('.action-pin') as HTMLButtonElement;
    ...
    this.headerActionRename?.addEventListener('click', (e) => {
      ...
      if (activeItem) {
        ...
        const renameBtn = activeItem.querySelector('.action-rename') as HTMLButtonElement;
    ...
    this.headerActionDelete?.addEventListener('click', (e) => {
      ...
      const activeItem = document.querySelector('.history-item.active') as HTMLElement;
      if (activeItem) {
        const deleteBtn = activeItem.querySelector('.action-delete') as HTMLButtonElement;
